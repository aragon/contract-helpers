...
module.exports = (web3) => {
  async function assertRole(acl, app, manager, roleName, grantee = manager) {
    ...
    const managerAddress = await acl.getPermissionManager(
      app.address,
      permission
    )

    assert.equal(
      web3.toChecksumAddress(managerAddress),
      web3.toChecksumAddress(manager.address),
  ...
  async function assertRoleNotGranted(acl, app, roleName, to) {
    ...
    assert.isFalse(
      ...
      `Given address should not have ${appName} role ${roleName}`
    )
  }
  ...
  async function assertMissingRole(acl, app, roleName) {
    ...
    assert.equal(
      managerAddress,
      ZERO_ADDRESS,
      `${appName} ${roleName} does have a manager set up`
    )
  }
  ...
  async function assertBurnedRole(acl, app, roleName) {
    ...
    assert.equal(
      managerAddress,
      burnEntity,
      `${appName} ${roleName} manager should be burned`
    )
  }
